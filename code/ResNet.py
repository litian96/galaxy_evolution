...
class ResNet(object):
    ...
    def build_network(self, images, is_training, reuse):
        with tf.variable_scope('ResNet', reuse=reuse):
            ...
            fc = fully_connected('fc', tf.reshape(r5, [batch_size, -1]), 1)
            return fc
    ...
    def build_train_op(self):
        ...
        logits = self.build_network(self.train_image_placeholder, True, False)
        loss = tf.reduce_mean(tf.square(self.train_label_placeholder - logits))
        ...
        lr_values = [0.0001, 0.0001, 0.0001, 0.00005, 0.00001]
        ...
        return train_op, self.train_loss, self.train_label_placeholder, logits
    
    def build_test_op(self):
        logits = self.build_network(self.test_image_placeholder, True, True)
        loss = tf.reduce_mean(tf.square(self.test_label_placeholder - logits))

        self.test_loss = loss

        return self.test_loss, self.test_label_placeholder, logits
